...
let hasWarnedMissingYelpKey = false

async function callYelp(path, params = '') {
  if (isPlaceholder(YELP_KEY, 'your-yelp-api-key-here')) {
    if (!hasWarnedMissingYelpKey) {
      hasWarnedMissingYelpKey = true
      console.warn('YELP_API_KEY missing or placeholder; skipping Yelp calls (warn once)')
    }
    return { businesses: [] }
  }
