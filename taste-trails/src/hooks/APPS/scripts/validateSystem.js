...
// Use a real city by default so validation can discover restaurants locally.
const TEST_CITY = process.env.VALIDATION_TEST_CITY || 'Charlotte';

// Increase overall timeout to allow discovery + scraping + persistence
...
async function checkRedis() {
  console.log(now(), 'Redis check skipped in single-server mode');
  return true;
}
...
async function runAll() {
  ...
  console.log(now(), 'Validation starting');
  console.log(now(), 'API_URL=', API_URL);
  console.log(now(), 'REDIS_URL=', REDIS_URL);
  console.log(now(), 'TEST_CITY=', TEST_CITY);
